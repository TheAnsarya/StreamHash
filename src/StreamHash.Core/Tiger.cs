...
/// <summary>
/// High-performance streaming implementation of the Tiger-192 cryptographic hash function.
/// </summary>
/// <remarks>
/// <para>
/// Tiger is a cryptographic hash function designed by Ross Anderson and Eli Biham in 1995,
/// optimized for 64-bit platforms. It processes data in 512-bit blocks and produces a
/// 192-bit hash value. The algorithm uses three passes over the data with different
/// multiplier constants and four large S-boxes (each 256 entries of 64-bit values).
/// </para>
/// <para>
/// <b>Algorithm Characteristics:</b>
/// <list type="bullet">
/// <item><b>Output Size:</b> 192 bits (24 bytes)</item>
/// <item><b>Block Size:</b> 512 bits (64 bytes)</item>
/// <item><b>Structure:</b> Custom Merkle-Damgård with key schedule</item>
/// <item><b>Passes:</b> 3 passes of 8 rounds each (24 rounds total)</item>
/// <item><b>Word Size:</b> 64-bit words, little-endian</item>
/// <item><b>S-boxes:</b> 4 S-boxes × 256 entries × 8 bytes = 8 KB</item>
/// </list>
/// </para>
/// <para>
/// <b>References:</b>
/// <list type="bullet">
/// <item><see href="https://www.cs.technion.ac.il/~biham/Reports/Tiger/">Tiger: A Fast New Hash Function (Anderson, Biham)</see></item>
/// </list>
/// </para>
/// </remarks>
public sealed class NativeTigerDigest : IStreamingHashBytes {
	private const int BlockSizeValue = 64;
	private const int DigestSizeValue = 24;
	...
	/// <summary>
	/// Creates a new Tiger-192 streaming hash instance.
	/// </summary>
	public NativeTigerDigest() {
		Reset();
	}
	...
	public byte[] FinalizeBytes() {
		// Tiger padding: append 0x01, then zeros, then 64-bit length (little-endian)
		// Message must be congruent to 56 mod 64
		...
		padding[0] = 0x01; // Tiger uses 0x01, not 0x80
		...
		BinaryPrimitives.WriteUInt64LittleEndian(result.AsSpan(0), _a);
		BinaryPrimitives.WriteUInt64LittleEndian(result.AsSpan(8), _b);
		BinaryPrimitives.WriteUInt64LittleEndian(result.AsSpan(16), _c);
		return result;
	}
	...
	private void ProcessBlock(ReadOnlySpan<byte> block) {
		...
		for (int i = 0; i < 8; i++) {
			x[i] = BinaryPrimitives.ReadUInt64LittleEndian(block.Slice(i * 8));
		}

		ulong a = _a, b = _b, c = _c;

		// Pass 1 (mul = 5)
		Pass(ref a, ref b, ref c, x, 5);
		KeySchedule(x);

		// Pass 2 (mul = 7)
		Pass(ref c, ref a, ref b, x, 7);
		KeySchedule(x);

		// Pass 3 (mul = 9)
		Pass(ref b, ref c, ref a, x, 9);
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static void Pass(ref ulong a, ref ulong b, ref ulong c, Span<ulong> x, ulong mul) {
	...
	/// <summary>
	/// Tiger key schedule: generates new message words from the previous block words.
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	...
	#region Tiger S-boxes

	// The Tiger S-boxes are four tables of 256 64-bit values each.
	// These are the canonical values from the Tiger reference implementation.
...
/// <summary>
/// Factory methods for creating native Tiger-192 streaming hash instances.
/// </summary>
internal static class TigerFactory {
	/// <summary>
	/// Creates a Tiger-192 streaming hash instance.
	/// </summary>
	/// <returns>A new Tiger-192 streaming hash.</returns>
	public static IStreamingHashBytes CreateTiger192() => new NativeTigerDigest();

	/// <summary>
	/// Computes Tiger-192 hash in one shot with minimal allocations.
	/// </summary>
	/// <param name="data">The data to hash.</param>
	/// <returns>The 24-byte Tiger-192 hash.</returns>
	[MethodImpl(MethodImplOptions.AggressiveOptimization)]
	public static byte[] ComputeTiger192(ReadOnlySpan<byte> data) {
		using var hasher = new NativeTigerDigest();
		hasher.Update(data);
