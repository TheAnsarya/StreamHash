...
public sealed class NativeBlake2bDigest : IStreamingHashBytes {
	...
	/// <summary>
	/// BLAKE2 message schedule permutation sigma.
	/// </summary>
	...
	private void Compress(ReadOnlySpan<byte> block, bool isFinal) {
		...
		v[12] = IV[4] ^ _t0;
		v[13] = IV[5] ^ _t1;
		v[14] = isFinal ? IV[6] ^ 0xffffffffffffffff : IV[6];
		v[15] = IV[7];

		// 12 rounds of mixing
		for (int round = 0; round < Rounds; round++) {
			G(v, m, round, 0, 4, 8, 12, 0);
			G(v, m, round, 1, 5, 9, 13, 1);
			G(v, m, round, 2, 6, 10, 14, 2);
			G(v, m, round, 3, 7, 11, 15, 3);
			G(v, m, round, 0, 5, 10, 15, 4);
			G(v, m, round, 1, 6, 11, 12, 5);
			G(v, m, round, 2, 7, 8, 13, 6);
			G(v, m, round, 3, 4, 9, 14, 7);
		}
	...
	private static void G(Span<ulong> v, Span<ulong> m, int round, int a, int b, int c, int d, int i) {
		int s0 = Sigma[round, 2 * i];
		int s1 = Sigma[round, 2 * i + 1];
...
public sealed class NativeBlake2sDigest : IStreamingHashBytes {
	...
	private void Compress(ReadOnlySpan<byte> block, bool isFinal) {
		...
		v[12] = IV[4] ^ _t0;
		v[13] = IV[5] ^ _t1;
		v[14] = isFinal ? IV[6] ^ 0xffffffff : IV[6];
		v[15] = IV[7];
	...
	/// <summary>
	/// BLAKE2s G mixing function. Rotation constants: 16, 12, 8, 7.
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
